...
public class LeiZhenZiTower : CallTower {
    ...
    protected override void SpawnBullet(string type, EventObject eventObject)
    {
        if (eventObject.name == "generateBullet")
        {
            ...
            GameObject go = Game.Instance.a_ObjectPool.Spawn("LeiZhenZiBullet");
            LeiZhenZiBullet bullet = go.GetComponent<LeiZhenZiBullet>();
            if(m_Target != null)
            {
