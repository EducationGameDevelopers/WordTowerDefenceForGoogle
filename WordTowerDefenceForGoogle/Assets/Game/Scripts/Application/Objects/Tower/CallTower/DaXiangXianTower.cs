...
using UnityEngine;
using DragonBones;
public class DaXiangXianTower : CallTower {
    ...
    protected override void SpawnBullet(string type, EventObject eventObject)
    {
        if (eventObject.name == "generateBullet")
        {
            ...
            GameObject go = Game.Instance.a_ObjectPool.Spawn("DaXiangXianBullet");
            DaXiangXianBullet bullet = go.GetComponent<DaXiangXianBullet>();
            if (m_Target != null)
            {
