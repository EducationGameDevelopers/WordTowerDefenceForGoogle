...
public class GuiXianRenTower : CallTower {
    ...
    protected override void SpawnBullet(string type, EventObject eventObject)
    {
        if (eventObject.name == "generateBullet")
        {
            ...
            GameObject go = Game.Instance.a_ObjectPool.Spawn("GuiXianRenBullet");
            GuiXianRenBullet bullet = go.GetComponent<GuiXianRenBullet>();
            if (m_Target != null)
            {
