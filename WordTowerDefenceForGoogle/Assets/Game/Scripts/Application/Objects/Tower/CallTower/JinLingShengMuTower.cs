...
using UnityEngine;
using DragonBones;
public class JinLingShengMuTower : CallTower {
    ...
    protected override void SpawnBullet(string type, EventObject eventObject)
    {
        if (eventObject.name == "generateBullet")
        {
            ...
            GameObject go = Game.Instance.a_ObjectPool.Spawn("JinLingShengMuBullet");
            JinLingShengMuBullet bullet = go.GetComponent<JinLingShengMuBullet>();
            if (m_Target != null)
            {
